class Autocomplete {
  ...
  getMatches( text ) {
    let arrObjects = [];
    const select = this.input.getElementsByTagName('option');

    for (let i = 0; i < select.length; i++) {
      if (select[i].textContent.includes(text)) {
        arrObjects.push({text: select[i].textContent, value: select[i].value});
      }
    }
